...
import sys as __icn_sys, os as __icn_os
__icn_icon_file = "icon.ico"
def __icn_set_icon():
    try:
        for __icn_mod in ['PySide6', 'PyQt6', 'PySide2', 'PyQt5']:
            try:
                __icn_wid = __import__(f'{__icn_mod}.QtWidgets', fromlist=['QApplication'])
                __icn_app = __icn_wid.QApplication.instance()
                if __icn_app is not None:
                    __icn_gui = __import__(f'{__icn_mod}.QtGui', fromlist=['QIcon'])
                    p = __icn_os.path.join(__icn_sys._MEIPASS, __icn_icon_file) if getattr(__icn_sys, 'frozen', False) else __icn_icon_file
                    if __icn_os.path.exists(p):
                        __icn_app.setWindowIcon(__icn_gui.QIcon(p))
                break
            except Exception:
                continue
    except Exception:
...
for __icn_mod in ['PySide6', 'PyQt6', 'PySide2', 'PyQt5']:
    try:
        __icn_core = __import__(f'{__icn_mod}.QtCore', fromlist=['QTimer'])
        __icn_core.QTimer.singleShot(0, __icn_set_icon)
        break
    except Exception:
        continue
