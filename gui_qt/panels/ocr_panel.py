...
class OcrPanelPage(BaseQtPanel, TaskPanelMixin):
    ...
    def build(self):
        ...
        row.addWidget(CaptionLabel("chi_sim=简体中文  eng=英文  jpn=日文  kor=韩文"))
        row.addStretch(1)
        sec.add_layout(row)
        lay.addWidget(sec)
        ...
        self.txt_result.setPlaceholderText("识别完成后在此显示文字…")
    ...
    def _export_txt(self):
        text = self.txt_result.toPlainText().strip()
        if not text:
            toast.show_warning(self, "暂无识别结果")
            return
        from PySide6.QtWidgets import QFileDialog
        path, _ = QFileDialog.getSaveFileName(
            self, "导出识别结果", "ocr_result.txt", "文本文件 (*.txt)")
        if path:
            try:
